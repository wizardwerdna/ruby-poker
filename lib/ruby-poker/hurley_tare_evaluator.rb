require File.expand_path('../hurley_value', __FILE__)

class HurleyTareEvaluator
    include Hurley
	...
	def royal_flush?
        if (md = (@hand.by_suit =~ /A(.) K\1 Q\1 J\1 T\1/))
          HurleyScore.new [[10], arrange_hand(md)]
        else
          false
        end
	end
	...
	def straight_flush?
        if (md = (/.(.)(.)(?: 1.\2){4}/.match(delta_transform(true))))
          ...
          HurleyScore.new [[9, high_card], arranged_hand]
        else
          false
        end
	end
	...
	def straight?
        ...
        if @hand.hand.size >= 5
          ...
          if (md = (/.(.). 1.. 1.. 1.. 1../.match(transform)))
            ...
            result = HurleyScore.new [[5, high_card], arranged_hand]
          end
        end
        result
	end
	...
	def highest_card?
        result = @hand.by_face
        HurleyScore.new [[1, *result.face_values[0..4]], result.hand.join(' ')]
	end
