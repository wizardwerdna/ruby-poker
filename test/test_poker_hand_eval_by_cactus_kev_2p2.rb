...
class TestCactusKevEvalByCactusKev2p2 < Test::Unit::TestCase
  context "A PokerHand instance evaluated by Cactus-Kev with ruby hash lookups" do
  
    setup do
      @trips = PokerHand.new("2D 9C AS AH AC", CactusKev2p2Evaluator)
      @full_boat = PokerHand.new(["2H", "2D", "4C", "4D", "4S"], CactusKev2p2Evaluator)
      @flush = PokerHand.new("3D 6D 7D TD QD AS AH`", CactusKev2p2Evaluator)
      @straight = PokerHand.new("8H 9D TS JH QC ", CactusKev2p2Evaluator)
    end
    ...
    # 
    #   assert_equal("As Ah 3d 3c Kd", PokerHand.new("AS AH KD 3D 3C", CactusKev2p2Evaluator).sort_using_rank)
    #   assert_equal("As Ah 3d 3c 2d", PokerHand.new("2D AS AH 3D 3C", CactusKev2p2Evaluator).sort_using_rank)
    # end
    ...
    should "recognize a straight flush" do
      ...
      assert PokerHand.new("8H 9H TH JH QH", CactusKev2p2Evaluator).straight_flush?
    end
  
    should "recognize a royal flush" do
      assert !@flush.royal_flush?
      assert PokerHand.new("AD KD QD JD TD", CactusKev2p2Evaluator).royal_flush?
    end
    ...
    should "recognize a four of a kind" do
      ...
      assert PokerHand.new("AD 9C AS AH AC", CactusKev2p2Evaluator)
    end
    ...
    should "recognize a straight" do
      ...
      assert PokerHand.new("2S 3D 4H 5D 6H", CactusKev2p2Evaluator).straight?
      assert PokerHand.new("AH 2S 3D 4H 5D", CactusKev2p2Evaluator).straight?
    end
    ...
    should "recognize a two pair" do
      assert PokerHand.new("2S 2D TH TD 4S", CactusKev2p2Evaluator).two_pair?
      assert !PokerHand.new("6D 7C 5D 5H 3S", CactusKev2p2Evaluator).two_pair?
    end
    
    should "recognize a pair" do
      assert !PokerHand.new("5C JC 2H 7S 3D", CactusKev2p2Evaluator).pair?
      assert PokerHand.new("6D 7C 5D 5H 3S", CactusKev2p2Evaluator).pair?
    end
    
    should "recognize a hand with the rank highest_card" do
      # hard to test, make sure it does not return null
      assert PokerHand.new("2D 4S 6C 8C TH", CactusKev2p2Evaluator).highest_card?
    end
    ...
    should "return the correct number of cards in the hand" do
      ...
      assert_equal(1, PokerHand.new("2c", CactusKev2p2Evaluator).size)
      assert_equal(2, PokerHand.new("2c 3d", CactusKev2p2Evaluator).size)
    end
  
    should "be comparable to other PokerHands" do
      hand1 = PokerHand.new("5C JC 2H 5S 3D", CactusKev2p2Evaluator)
      hand2 = PokerHand.new("6D 7C 5D 5H 3S", CactusKev2p2Evaluator)
    ...
    should "be considered equal to other poker hands that contain the same cards" do
      ...
      hand1 = PokerHand.new("Ac Qc Ks Kd 9d", CactusKev2p2Evaluator)
      hand2 = PokerHand.new("Ah Qs 9h Kh Kc", CactusKev2p2Evaluator)
      assert_equal(0, hand1 <=> hand2)
    end
  
    should "be able to add a Card to itself" do
      ph = PokerHand.new("", CactusKev2p2Evaluator)
      ph << "Qd"
      ph << Card.new("2D")
      ph << ["3d", "4d"]
    ...
    should "be able to delete a card" do
      ph = PokerHand.new("Ac", CactusKev2p2Evaluator)
      ph.delete("Ac")
      assert_equal(Array.new, ph.hand)
    end
    
    # should "detect the two highest pairs when there are more than two" do
    #   ph = PokerHand.new("7d 7s 4d 4c 2h 2d", CactusKev2p2Evaluator)
    ...
    #     uniq_ph = PokerHand.new("3s 4s 3s", CactusKev2p2Evaluator).uniq
