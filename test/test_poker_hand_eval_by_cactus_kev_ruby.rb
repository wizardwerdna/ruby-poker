...
class TestCactusKevEvalByCactusKevRuby < Test::Unit::TestCase
  context "A PokerHand instance evaluated by Cactus-Kev with ruby hash lookups" do
  
    setup do
      @trips = PokerHand.new("2D 9C AS AH AC", CactusKevRubyEvaluator)
      @full_boat = PokerHand.new(["2H", "2D", "4C", "4D", "4S"], CactusKevRubyEvaluator)
      @flush = PokerHand.new("3D 6D 7D TD QD AS AH`", CactusKevRubyEvaluator)
      @straight = PokerHand.new("8H 9D TS JH QC ", CactusKevRubyEvaluator)
    end
    ...
    # 
    #   assert_equal("As Ah 3d 3c Kd", PokerHand.new("AS AH KD 3D 3C", CactusKevRubyEvaluator).sort_using_rank)
    #   assert_equal("As Ah 3d 3c 2d", PokerHand.new("2D AS AH 3D 3C", CactusKevRubyEvaluator).sort_using_rank)
    # end
    ...
    should "recognize a straight flush" do
      ...
      assert PokerHand.new("8H 9H TH JH QH", CactusKevRubyEvaluator).straight_flush?
    end
  
    should "recognize a royal flush" do
      assert !@flush.royal_flush?
      assert PokerHand.new("AD KD QD JD TD", CactusKevRubyEvaluator).royal_flush?
    end
    ...
    should "recognize a four of a kind" do
      ...
      assert PokerHand.new("AD 9C AS AH AC", CactusKevRubyEvaluator)
    end
    ...
    should "recognize a straight" do
      ...
      assert PokerHand.new("2S 3D 4H 5D 6H", CactusKevRubyEvaluator).straight?
      assert PokerHand.new("AH 2S 3D 4H 5D", CactusKevRubyEvaluator).straight?
    end
    ...
    should "recognize a two pair" do
      assert PokerHand.new("2S 2D TH TD 4S", CactusKevRubyEvaluator).two_pair?
      assert !PokerHand.new("6D 7C 5D 5H 3S", CactusKevRubyEvaluator).two_pair?
    end
    
    should "recognize a pair" do
      assert !PokerHand.new("5C JC 2H 7S 3D", CactusKevRubyEvaluator).pair?
      assert PokerHand.new("6D 7C 5D 5H 3S", CactusKevRubyEvaluator).pair?
    end
    
    should "recognize a hand with the rank highest_card" do
      # hard to test, make sure it does not return null
      assert PokerHand.new("2D 4S 6C 8C TH", CactusKevRubyEvaluator).highest_card?
    end
    ...
    should "return the hand's rating as a string" do
      assert_equal "Three Aces", @trips.hand_rating
      assert_equal "Fours Full over Deuces", @full_boat.hand_rating
    end
    ...
    should "return the hand's score" do
      assert_equal(CactusKev::EqClTable[1654], @trips.score)
    end
    ...
    should "return the correct number of cards in the hand" do
      ...
      assert_equal(1, PokerHand.new("2c", CactusKevRubyEvaluator).size)
      assert_equal(2, PokerHand.new("2c 3d", CactusKevRubyEvaluator).size)
    end
  
    should "be comparable to other PokerHands" do
      hand1 = PokerHand.new("5C JC 2H 5S 3D", CactusKevRubyEvaluator)
      hand2 = PokerHand.new("6D 7C 5D 5H 3S", CactusKevRubyEvaluator)
    ...
    should "be considered equal to other poker hands that contain the same cards" do
      ...
      hand1 = PokerHand.new("Ac Qc Ks Kd 9d", CactusKevRubyEvaluator)
      hand2 = PokerHand.new("Ah Qs 9h Kh Kc", CactusKevRubyEvaluator)
      assert_equal(0, hand1 <=> hand2)
    end
  
    should "be able to add a Card to itself" do
      ph = PokerHand.new("", CactusKevRubyEvaluator)
      ph << "Qd"
      ph << Card.new("2D")
      ph << ["3d", "4d"]
    ...
    should "be able to delete a card" do
      ph = PokerHand.new("Ac", CactusKevRubyEvaluator)
      ph.delete("Ac")
      assert_equal(Array.new, ph.hand)
    end
    
    # should "detect the two highest pairs when there are more than two" do
    #   ph = PokerHand.new("7d 7s 4d 4c 2h 2d", CactusKevRubyEvaluator)
    ...
    #     uniq_ph = PokerHand.new("3s 4s 3s", CactusKevRubyEvaluator).uniq
